    ...
    <section className="w-full bg-secondary py-28 px-16">
      <div className="max-w-[768px] mx-auto flex flex-col gap-12">
        <div className="flex flex-col gap-4">
          <h2 className="text-4xl font-black text-foreground uppercase tracking-tight">FAQ</h2>
          <p className="text-lg text-muted-foreground">Common questions about GLYCO8™</p>
        </div>
        <div className="flex flex-col">
          {faqs.map((faq, i) => (
            <div key={i} className="border-b border-border">
              ...
              {open === i && <p className="pb-5 text-base text-muted-foreground">{faq.a}</p>}
            </div>
          ))}
        </div>
        <div>
          <h4 className="text-lg font-bold text-foreground">Still have questions?</h4>
