...
const FAQ = () => {
  ...
  return (
    <section className="w-full bg-muted py-28 px-16">
      <div className="max-w-[768px] mx-auto flex flex-col gap-12">
        <div className="flex flex-col gap-4">
          <h2 className="text-5xl font-bold leading-[1.2] text-foreground">FAQs</h2>
          <p className="text-lg text-foreground">Everything you need to know about our products and protocols.</p>
        </div>
        <div className="flex flex-col">
          {faqs.map((faq, i) => (
            <div key={i} className="border-b border-foreground/20">
              <button
                className="w-full flex items-center justify-between py-5 text-left"
                onClick={() => setOpen(open === i ? null : i)}
              >
                <span className="text-base font-semibold text-foreground">{faq.q}</span>
                <ChevronDown className={`w-5 h-5 text-foreground transition-transform ${open === i ? "rotate-180" : ""}`} />
              </button>
              {open === i && (
        ...
        <div>
          ...
          <p className="text-base text-muted-foreground mt-1">Contact our support team for personalized assistance.</p>
        </div>
      </div>
    </section>
  );
